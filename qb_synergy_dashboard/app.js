...
(() => {
  ...
  function scoreBreakdown(row, weekly = false) {
    ...
    }).join('')}</div><div class="score-penalties"><p>Interception penalty <b>${fmt(detail.turnoverPenalty,2)} / 3</b></p><p>Failed 3rd/4th-down penalty <b>${fmt(detail.moneyPenalty,2)} / 2</b></p></div><p class="score-equation">${fmt(total,2)} component points − ${fmt(detail.penalty,2)} penalty points = <strong style="color:${S.color(row.synergyScore)}">${fmt(row.synergyScore)} / 100</strong> <span>(rounded)</span></p><p class="score-context">${detail.peers} eligible ${S.group(row)==='RB'?'RB':'WR/TE'} peers${weekly?' from this week':''}; ${detail.minimum}+ ${detail.minimum===1?'target':'targets'}. Efficiency is stabilized toward the ${weekly?'weekly ':''}peer baseline using targets / (targets + ${weekly?8:40}). Missing model inputs suppress the composite rather than counting as zero.</p>`;
  }
