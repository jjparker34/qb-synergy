...
(() => {
  ...
  function renderExplorer() {
    ...
    $('#pairList').innerHTML = pairs.map((r,i)=>`<button class="pair ${S.id(row)===S.id(r)?'active':''}" data-pair="${i}" ${S.id(row)===S.id(r)?'aria-pressed="true"':'aria-pressed="false"'}>${photo(r,'receiver')}<span><span class="pair-name">${esc(full(r,'receiver'))}</span><small>${r.position} · ${r.team} · ${fmt(r.targets)} targets</small></span><b>${fmt(r.synergyScore)}</b></button>`).join('');
